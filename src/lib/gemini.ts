...
export async function extractMeterReading(image: { mimeType: string; data: string }): Promise<number | null> {
  ...
  try {
    const res = await model.generateContent([
      {
        text: `Đây là ảnh đồng hồ điện (công tơ điện). Hãy đọc chỉ số điện hiển thị trên mặt đồng hồ.
Trả về JSON với duy nhất một khoá: "reading" là số nguyên chỉ số kWh đọc được (chỉ lấy phần nguyên, bỏ phần thập phân nếu có).
Nếu không đọc được hoặc không phải ảnh đồng hồ điện, trả về {"reading": null}.
Chỉ trả JSON thuần, không markdown.`,
      },
      { inlineData: { mimeType: image.mimeType, data: image.data } },
    ]);
    const parsed = JSON.parse(res.response.text()) as { reading: number | null };
    return typeof parsed.reading === "number" ? Math.round(parsed.reading) : null;
  } catch {
    return null;
  }
}
