...
import { ArrowUpCircle, Pencil, Trash2 } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <Select value={filterParty} onValueChange={setFilterParty}>
          ...
          <SelectContent>
            ...
            {partyKindConfigs.filter((p) => p.forExpense).map((p) => <SelectItem key={p.code} value={p.code}>{p.label}</SelectItem>)}
          </SelectContent>
        </Select>
        <Select value={filterCategory} onValueChange={setFilterCategory}>
          <SelectTrigger className="w-[160px]"><SelectValue placeholder="Loại chi" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="ALL">Tất cả loại chi</SelectItem>
            {categories.filter((c) => c.type === "EXPENSE").map((c) => <SelectItem key={c.id} value={c.name}>{c.name}</SelectItem>)}
          </SelectContent>
        </Select>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <MiniStat label="Phải trả" value={formatVND(totalDue)} variant="sage" />
        <MiniStat label="Đã trả" value={formatVND(totalPaid)} variant="accent" />
...
function DebtCard({ r, canWrite, onEdit, onDelete, buildingId, contractMap, invoiceMap }: {
  r: Row;
  canWrite: boolean;
  onEdit: () => void;
  onDelete: () => void;
    ...
    <Card>
      <CardContent className="p-3 space-y-2">
        ...
        <div className="grid grid-cols-4 gap-1.5 text-[11px]">
          ...
          <CardStat label="Phải trả" value={formatVND(BigInt(r.payable))} />
          <CardStat label="Đã trả" value={formatVND(BigInt(r.paid))} />
...
function CardStat({ label, value, danger, bold }: { label: string; value: string; danger?: boolean; bold?: boolean }) {
  const color = danger ? "text-rose-600" : "";
  return (
    <div>
